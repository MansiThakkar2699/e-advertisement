...
import { Search, Eye, Trash2, CheckCircle, Loader2, Ban, ShieldCheck, PlusCircle, PauseCircle, Clock, XCircle, Check, X, FolderPlus, Edit2 } from 'lucide-react';
import DataTable from '../DataTable'; // Your reusable component
import { toast } from 'react-toastify';
import AdvertisementBuilderModal from "./AdvertisementBuilderModal";
...
const AdvertisementPage = () => {
    ...
    const fetchAdvertisements = async () => {
        try {
            ...
            const response = await axios.get(`/ads/advertisement/advertiser/${decoded.id}`,
                {
                    headers: {
    ...
    const fetchCampaigns = async () => {
        try {
            ...
            const response = await axios.get(
                `/campaign/campaign/advertiser/${decoded.id}`,
                {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
            ...
            key: 'ad_title', label: 'Advertisement Details', sortable: true, render: (_, advertisement) => (
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-medium text-sm">
                        {advertisement.campaign_details?.name.split(' ').map(n => n[0]).join('')}
                    </div>
                    <div>
                        <p className="font-semibold text-slate-900 text-medium">{advertisement.ad_title}</p>
                        <p className="text-slate-500 text-sm">{advertisement.campaign_details?.name}</p>
                    </div>
                </div>
            )
        },
        ...
        <div className="p-8 bg-slate-50 min-h-screen">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                ...
                <div className="flex gap-3">
                    <button onClick={() => setOpenAddModal(true)}
                        ...
                        <FolderPlus className="w-4 h-4" /> Add New Advertisement
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200 mb-6 flex flex-col md:flex-row gap-4 items-center">
                ...
                <div className="flex gap-2 w-full md:w-auto">
                    <select
                        className="bg-slate-50 border border-slate-200 text-slate-600 text-sm rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-500"
                ...
                <AdvertisementBuilderModal
                    ...
                    campaigns={campaigns.filter(
                        (campaign) =>
                            ...
                            campaign._id === selectedAdvertisement?.campaign_details?._id ||
                            campaign._id === selectedAdvertisement?.campaign_id?._id ||
                            campaign._id === selectedAdvertisement?.campaign_id
                    )}
                />
            )}
        </div>
